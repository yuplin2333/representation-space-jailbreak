...
def compute_between_within_variance(pca, dataset_embeddings, labels, num_components):
    # Transform the embeddings to PCA space
    transformed_embeddings = [pca.transform(embed.cpu().numpy()) for embed in dataset_embeddings]

    # Separate the transformed embeddings into the first two dimensions and the remaining dimensions
    transformed_embeddings_first_two = [embed[:, :2] for embed in transformed_embeddings]
    transformed_embeddings_remaining = [embed[:, 2:num_components] for embed in transformed_embeddings]

    # Calculate between-class and within-class variance for the first two dimensions
    between_class_variance_first_two, within_class_variance_first_two = calculate_variances(transformed_embeddings_first_two)
    total_variance_first_two = between_class_variance_first_two + within_class_variance_first_two
    between_class_ratio_first_two = between_class_variance_first_two / total_variance_first_two
    within_class_ratio_first_two = within_class_variance_first_two / total_variance_first_two

    # Calculate between-class and within-class variance for the remaining dimensions
    between_class_variance_remaining, within_class_variance_remaining = calculate_variances(transformed_embeddings_remaining)
    total_variance_remaining = between_class_variance_remaining + within_class_variance_remaining
    between_class_ratio_remaining = between_class_variance_remaining / total_variance_remaining
    within_class_ratio_remaining = within_class_variance_remaining / total_variance_remaining

    return (between_class_variance_first_two, within_class_variance_first_two, total_variance_first_two,
            between_class_ratio_first_two, within_class_ratio_first_two,
            between_class_variance_remaining, within_class_variance_remaining, total_variance_remaining,
            between_class_ratio_remaining, within_class_ratio_remaining)


def calculate_variances(transformed_embeddings):
...
def main():
    ...
    (between_variance_first_two, within_variance_first_two, total_variance_first_two,
     between_ratio_first_two, within_ratio_first_two,
     between_variance_remaining, within_variance_remaining, total_variance_remaining,
     between_ratio_remaining, within_ratio_remaining) = compute_between_within_variance(
        pca, hidden_states_datasets_list, args.labels, args.num_components
    )

    print(f"==>> First two dimensions - Between-class variance: {between_variance_first_two}")
    print(f"==>> First two dimensions - Within-class variance: {within_variance_first_two}")
    print(f"==>> First two dimensions - Total variance: {total_variance_first_two}")
    print(f"==>> First two dimensions - Between-class variance ratio: {between_ratio_first_two:.4f}")
    print(f"==>> First two dimensions - Within-class variance ratio: {within_ratio_first_two:.4f}")

    print(f"==>> Remaining dimensions - Between-class variance: {between_variance_remaining}")
    print(f"==>> Remaining dimensions - Within-class variance: {within_variance_remaining}")
    print(f"==>> Remaining dimensions - Total variance: {total_variance_remaining}")
    print(f"==>> Remaining dimensions - Between-class variance ratio: {between_ratio_remaining:.4f}")
    print(f"==>> Remaining dimensions - Within-class variance ratio: {within_ratio_remaining:.4f}")
