...
for i in "${!model_repos[@]}"; do
    ...
    for start_idx in $(seq "$start" "$step" "$((end - 1))"); do
        ...
        output_dir="$base_output_dir/ours_gcg/${model_nickname}"
        job_name="ours_gcg_${model_nickname}_${start_idx}_${end_idx}"
        log_path="$base_log_dir/attack/ours_gcg/${model_nickname}/idx_${start_idx}_${end_idx}.log"
        ...
        sbatch --job-name=$job_name --ntasks=1 --cpus-per-task=8 --gpus=a100:1 --mem=32G --time=0-04:00:00 --output=$log_path \
            scripts/directrun_ours_gcg.sh $model_repo $start_idx $end_idx $output_dir
    done
done
