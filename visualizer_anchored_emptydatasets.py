...
def main():
    ...
    if args.datasets:
        for idx, dataset_path in enumerate(args.datasets):
            ...
            valid_text_columns.append(column_name)
            valid_colors.append(args.colors[idx + 2])  # Skip the first two colors for anchors
            valid_labels.append(args.labels[idx + 2])  # Skip the first two labels for anchors

    print(f"==> SAMPLE: {df_texts_list[0][0]}" if df_texts_list else "==> No datasets provided")
    ...
    embedding_labels = args.labels[:2] + valid_labels

    # Update colors to remove those corresponding to skipped datasets
    color_list = list(args.colors[:2]) + valid_colors
    ...
    for idx, hidden_states_dataset in enumerate(hidden_states_datasets_list):
        ...
        print(f"Length of hidden_states_dataset: {hidden_states_dataset.shape[0]}")
