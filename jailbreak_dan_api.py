...
def jailbreak_this_prompt(prompt, model_api, model_judge, tokenizer_judge, dan):
    ...
    # response definitely contains refusal at the beginning, because DAN gives affirmative after "[🔓JAILBREAK]"
    # First, we check if there is a "[🔓JAILBREAK]" in the response
    if "[🔓JAILBREAK]" in response:
        # We check the part after "[🔓JAILBREAK]" to see if it contains refusal
        response = response.split("[🔓JAILBREAK]")[1]
    else:
